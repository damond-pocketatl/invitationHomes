...
class PMXI_Import_Record extends PMXI_Model_Record {
	...
	public function process($xml, $logger = NULL, $chunk = false, $is_cron = false) {
		...
		try { 						
			...
			foreach ($titles as $i => $void) {					
				...
				if (!empty($chunk_records) and $this->large_import == 'Yes' and $this->options['create_chunks'] and !$is_cron and !in_array($_SESSION['pmxi_import']['chunk_number'], $chunk_records)) {
					...
					if ( ! $is_cron ){
						$records_count = $_SESSION['pmxi_import']['created_records'] + $_SESSION['pmxi_import']['updated_records'] + $_SESSION['pmxi_import']['skipped_records'] + $_SESSION['pmxi_import']['errors'];
						$progress_msg = '<p class="import_process_bar"> '. __('Created','pmxi_plugin') . ' ' . $_SESSION['pmxi_import']['created_records'] . ' / ' . __('Updated','pmxi_plugin') . ' ' . $_SESSION['pmxi_import']['updated_records'] . ' of '. $_SESSION['pmxi_import']['count'].' records.</p><span class="import_percent">' . ceil(($records_count/$_SESSION['pmxi_import']['count']) * 100) . '</span><span class="warnings_count">' .  $_SESSION['pmxi_import']['warnings'] . '</span><span class="errors_count">' . $_SESSION['pmxi_import']['errors'] . '</span>';
						$logger and call_user_func($logger, $progress_msg);
					}						
					$this->set(array(
				...
				if ($post_to_update){
					...
					if ( ! $this->options['is_keep_images'] and ! $this->options['is_keep_attachments_on_update']){ 						
								...
								if (!empty($imgs)) {								
									foreach ($imgs as $img_url) { if (empty($img_url)) continue;									
										...
										if (base64_decode($img_url, true) !== false){
											...
											$image_filename = wp_unique_filename($uploads['path'], (($this->options['auto_rename_images'] and "" != $auto_rename_images[$i]) ? url_title($auto_rename_images[$i] . '_' . (($this->options['images_name'] != 'auto') ? basename($img_url) : uniqid())) : (($this->options['images_name'] != 'auto') ? basename($img_url) : uniqid())) . (("" != $img_ext and $this->options['images_name'] == 'auto') ? '.'.$img_ext : ''));
			...
			$is_import_complete = ($records_count == $_SESSION['pmxi_import']['count'] + $_SESSION['pmxi_import']['skipped_records']);
		...
		!$is_cron and ($records_count == $_SESSION['pmxi_import']['count']) and $logger and call_user_func($logger, __('Cleaning temporary data...', 'pmxi_plugin'));
